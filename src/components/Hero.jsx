        ...
        <motion.div variants={container} initial="hidden" animate="show">
          ...
          <motion.h1 variants={item} className="text-3xl md:text-5xl font-black tracking-tight leading-tight mb-6">
            <EditableText id="hero-title" tag="span" editMode={editMode}>
              {t('hero.titre.1')}{' '}
              <span className="gradient-text">{t('hero.titre.2')}</span>
              <br />
              {t('hero.titre.3')}
              <br />
              <span className="gradient-text">{t('hero.titre.4')}</span>
            </EditableText>
          </motion.h1>
          ...
          <motion.div variants={item} className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              <EditableText id="hero-cta1" editMode={editMode}>{t('hero.cta1')}</EditableText>
            </button>
            <button
            ...
            >
              <EditableText id="hero-cta2" editMode={editMode}>{t('hero.cta2')}</EditableText>
            </button>
          </motion.div>
            ...
            {[
              ...
              <div key={stat.id} className="text-center">
                ...
                <div className="text-xs text-slate-500 mt-1 font-medium">
                  <EditableText id={`hero-${stat.id}-label`} editMode={editMode}>{t(`hero.${stat.id}.label`)}</EditableText>
                </div>
              </div>
            ))}
          </motion.div>
